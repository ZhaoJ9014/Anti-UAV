...
class Tracker:
    ...
    def run_antiuav_sequence(self, seq, debug=None):
        params = self.params
        ...
        output = self._det_track_sequence(tracker, yolo_model, seq)
        return output

    def _det_track_sequence(self, tracker, detector, seq):
        output = {'target_bbox': [],
                  'time': []}
        ...
        detection_flag = True
        for frame_num, frame_path in enumerate(seq.frames, start=1):
            image = self._read_image(frame_path)
            start_time = time.time()
            if detection_flag==True:
                out = tracker.initialize(image, detector) # detection & initialization
