    ...
    if pred_bb.shape[0] != anno_bb.shape[0]:
        if dataset == 'lasot':
            ...
            if pred_bb.shape[0] > anno_bb.shape[0]:
                ...
                pad = jt.zeros((anno_bb.shape[0] - pred_bb.shape[0], 4)).type_as(pred_bb)
...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    avg_overlap_all = jt.zeros((len(dataset), len(trackers)), dtype=jt.float64)
    ave_success_rate_plot_overlap = jt.zeros((len(dataset), len(trackers), threshold_set_overlap.numel()),
                                                dtype=jt.float32)
    ave_success_rate_plot_center = jt.zeros((len(dataset), len(trackers), threshold_set_center.numel()),
                                               dtype=jt.float32)
    ave_success_rate_plot_center_norm = jt.zeros((len(dataset), len(trackers), threshold_set_center.numel()),
                                                    dtype=jt.float32)
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        # Load anno
        anno_bb = jt.tensor(seq.ground_truth_rect)
        target_visible = jt.tensor(seq.target_visible, dtype=jt.uint8) if seq.target_visible is not None else None
        for trk_id, trk in enumerate(trackers):
            ...
            if os.path.isfile(results_path):
                pred_bb = jt.tensor(load_text(str(results_path), delimiter=('\t', ','), dtype=np.float64))
            else:
                if skip_missing_seq:
