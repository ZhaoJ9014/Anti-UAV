...
def merge_multiple_runs(eval_data):
    ...
    merged = jt.zeros(len(trackers), dtype=jt.uint8)
    ...
    ave_success_rate_plot_center_norm_merged = jt.stack(ave_success_rate_plot_center_norm_merged, dim=1)
...
def plot_got_success(trackers, report_name):
    ...
    auc_curve = jt.zeros((len(trackers), 101))
    scores = jt.zeros(len(trackers))
