...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    ave_success_rate_plot_overlap = jt.Var.zeros((len(dataset), len(trackers), threshold_set_overlap.numel()),
                                                dtype=jt.float32)
    ave_success_rate_plot_center = jt.Var.zeros((len(dataset), len(trackers), threshold_set_center.numel()),
                                               dtype=jt.float32)
    ave_success_rate_plot_center_norm = jt.Var.zeros((len(dataset), len(trackers), threshold_set_center.numel()),
                                                    dtype=jt.float32)
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        target_visible = jt.Var(seq.target_visible, dtype=jt.uint8) if seq.target_visible is not None else None
        for trk_id, trk in enumerate(trackers):
            ...
            if os.path.isfile(results_path):
                pred_bb = jt.Var(load_text(str(results_path), delimiter=('\t', ','), dtype=np.float64))
            else:
                if skip_missing_seq:
