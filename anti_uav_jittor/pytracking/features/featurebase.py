...
class FeatureBase:
    ...
    def get_feature(self, im: jt.Var):
        ...
        if self.normalize_power is not None:
            feat /= (jt.sum(feat.abs().view(feat.shape[0],1,1,-1)**self.normalize_power, dim=3, keepdim=True) /
...
class MultiFeatureBase(FeatureBase):
    ...
    def size(self, im_sz):
        ...
        if isinstance(im_sz, jt.Var):
            return TensorList([im_sz // s if sz is None else jt.Var([sz[0], sz[1]]) for sz, s in zip(self.output_size, self.stride())])
